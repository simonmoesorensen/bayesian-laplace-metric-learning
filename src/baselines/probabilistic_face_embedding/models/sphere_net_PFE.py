    ...
    with slim.arg_scope(
        ...
        with tf.variable_scope("SphereNet", [images], reuse=reuse):
            ...
            with slim.arg_scope(
                ...
                net = conv_module(
                    images, num_layers[0], num_kernels[0], scope="conv1"
                )
                print("module_1 shape:", [dim.value for dim in net.shape])

                net = conv_module(
                    net, num_layers[1], num_kernels[1], scope="conv2"
                )
                print("module_2 shape:", [dim.value for dim in net.shape])

                net = conv_module(
                    net, num_layers[2], num_kernels[2], scope="conv3"
                )
                print("module_3 shape:", [dim.value for dim in net.shape])

                net = conv_module(
                    net, num_layers[3], num_kernels[3], scope="conv4"
                )
                ...
                mu = slim.fully_connected(
                    ...
                    normalizer_params=batch_norm_params_last,
                    activation_fn=None,
                )
