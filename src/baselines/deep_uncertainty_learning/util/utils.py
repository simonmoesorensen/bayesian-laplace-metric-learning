...
class add_gaussian_noise(object):
    ...
    def __call__(self, img):
        if random.uniform(0, 1) < self.p:
            ...
            noisy_img = image_array + np.random.normal(
                self.mean, std, image_array.shape
            )
...
def evaluate_model(dataloader, multi_gpu, model, n_sample=10):

    model = model.cuda()  # unpackage model from DataParallel

    model.eval()  # switch to evaluation mode

    prec1_lst = []
    prec5_lst = []
    # Speed up with no grad
    # with torch.no_grad():
    for inputs, labels in tqdm(dataloader):
        ...
        for _ in range(n_sample):
            mu_dul, std_dul = model(inputs)

            epsilon = torch.randn_like(std_dul)
            features = mu_dul + epsilon * std_dul

            embeddings = l2_norm(features)
            prec1, prec5 = accuracy(embeddings, labels, topk=(1, 5))

            prec1_lst.append(prec1.item())
            prec5_lst.append(prec5.item())

            #

    print(f"Prec@1: {np.mean(prec1_lst)} +- {np.std(prec1_lst)}")
    print(f"Prec@5: {np.mean(prec5_lst)} +- {np.std(prec5_lst)}")
...
def test_model(train_set, test_set, model, data_device, batch_size, num_workers):
    ...
    train_embeddings, train_labels = get_all_embeddings(train_set, model, data_device, batch_size, num_workers)
    test_embeddings, test_labels = get_all_embeddings(test_set, model, data_device, batch_size, num_workers)
