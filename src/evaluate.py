...
def recall(ranks, pidxs, qidxs, ks):
    ...
    return recall_at_k
"""
def compute_recall_k(ranks, pidx, ks):
...
def compute_map_k(ranks, pidxs, k):
    return np.mean([apk(a,p,k) for a,p in zip(pidxs, ranks)])


def compute_rank(mu, sigma, samesource=True):
    
    # expected distance
    if sigma is not None:
        dist = torch.cdist(mu, mu)
        rank = torch.argsort(dist, dim=1)
        
    # euclidean distance
    else:
        
        dist = np.matmul(mu, mu.T)
