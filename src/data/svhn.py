...
class SVHNDataModule(LightningDataModule):
    ...
    def setup(self, stage=None):
        ...
        if stage == "fit" or stage is None:
            train = SVHN(self.data_dir, split="train", transform=self.transform)
            self.df_train, self.df_val = random_split(train, [70000, 3257])

        # Assign test dataset for use in dataloader(s)
        if stage == "test" or stage is None:
            self.df_test = SVHN(self.data_dir, split="test", transform=self.transform)
